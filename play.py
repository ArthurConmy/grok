...
for model_file in ["checkpoints/32_train.pt", "checkpoints/32_valid.pt"]:
    ...
    for x, y in train_data:
        indices = 97 * (x[:,0]) + x[:,1]
        print("Training confidence", get_average_confidence(model, x))
    for x, y in valid_data:
        print("Valid confidence", get_average_confidence(model, x))
